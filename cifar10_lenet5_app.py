...
def pre_pic(picName):
    ...
    reIm = img.resize((32,256),Image.ANTIALIAS)    #用消除锯齿的方法将原图调整为32*32（antialias：抗锯齿）
...
"""    reshaped_array = np.reshape(im_array, (                              #导入部分，更改参数的形状
    ...
    reshaped_array = im_array.reshape([1,32*256])
...
def restore_model(testPicArr):                                              # 定义重建模型并进行预测
    with tf.Graph().as_default() as g:
        ...
        predictValue = cifar10_lenet5_generateds.vec2text(y)                                       # 定义输入图片的预测值
